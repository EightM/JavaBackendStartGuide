...
После того как ты создал своё web приложение, вероятно ты захочешь предоставить к нему доступ для пользователей. А где пользователи там и логины и пароли. А где пароли там и безопасность, личные данные, кросс-авторизации и злобные хакеры. Чтобы не страдать от головной боли, для нас создали фреймворк SpringSecurity, который дает нам удобный API и абстракции (в который раз), чтобы сделать наше приложение безопасным и надежным. Подробно можно ознакомиться в [документации](https://spring.io/projects/spring-security) и (сюрприз!) в отличной книге «Spring Security in Action». Рекомендую.

...
- *В руководстве выбрана ветка развития Java backend разработчика, на что опирается большинство рекомендаций в статье.*

- *Руководство написано предполагая, что читатель не полный новичок в CS, подразумевая наличие базовых знаний*
- *Руководство намеренно написано в сдержанном неформальном стиле, в формате монолога. Все предлагаемые изменения должны соответствовать данному стилю.*
...
1. **Огромная база кода.** Если ты хочешь сделать, с большой вероятностью для этого уже есть библиотека;
...
3. **Гибкость.** Java это не только язык, но и платформа JVM. С использованием этой платформы написаны, например, Kotlin (привет, Android), Scala (привет, ФинТех) и Clojure (привет, эээ, страдание?). Всё это позволяет нам оседлав Java и потратив некоторое время, дрейфовать в сторону той предметной области которая наиболее интересна;
4. **Сообщество.** Если у вас появился вопрос, на него скорее всего ответили ещё в 2010. Если у тебя появилась гениальная идея проекта, на гитхабе скорее всего уже есть десяток различных реализаций. Новичкам всегда радостно помогут советами прочитать наконец Javadoc.
...
1. **Многословность**. Java многословна. То, что сейчас в пайтоне занимает одну строку, в Java займёт 10. Не такой большой минус в современном мире где есть Intellij и авто дополнение, плюс в язык добавляются модные фичи вроде `var` . Плюс в языках вроде Kotlin или Scala можно писать достаточно компактный код;
2. **Обилие устаревшего кода.** Java появилась в 1995 году и с тех пор было написано много кода. Чертовски много. Всё это нужно поддерживать, развивать и лелеять, так как бизнес очень не любит тратить деньги. Например Java 8 вышла в 2014 году, а согласно [опросу](https://www.jetbrains.com/ru-ru/lp/devecosystem-2020/java/) JetBrains в 2020 году, её доля составляет 75% (!). Так что залетев на работу, с высокой долей вероятности будешь писать на старой версии, в которой нет современного сахара, увы;
3. **Ограниченная сфера применения.** В современном (на момент 2021 года) мире, Java по настоящему блистает в качестве backend языка. Да, ты можешь писать mobile, но там развивается Kotlin. Да, ты можешь писать desktop, но люди выбирают electron. Локомотив хайпа находится в вебе, поэтому мы садимся именного в него.

## Обучение

Если хорошенько поискать, можно найти десятки статей о том, какие книги лучше читать, какие курсы лучше проходить. Заботливые люди рисуют дорожные карты и казалось бы, просто бери и следуй гайдам. Но проблема в том, что их слишком много! Современный мир backend разработки требует от тебя разбираться в куче вещей, и простой человек Василий, попробовав во всём этом разобраться очень скоро почувствует беспомощность, грусть и отчаяние. 

Однако, всё не так страшно. Учиться придется действительно очень много, однако мы постараемся отобрать только самое лучшие ~~зёрна~~ материалы.
...
- **Среда разработки.** Используй intellij IDEA community, скажешь спасибо потом. Из альтернатив есть: 
  - Eclipse. Старый, неудобный, но ещё относительно популярен в некоторых местах;
  - VS Code. Это не IDE, но редактор кода, который обвешивают кучей плагинов;
  - NetBeans. Когда-то очень давно была популярна, сейчас используется только если случайно скачал инсталлятор.
...
- **Профиль на GitHub**. Гитхаб это такая социальная сеть для разработчиков. Выкладывай туда все свои мелкие проекты, ставь лайки библиотекам которые используешь, изучай тренды. Не стесняйся выкладывать туда все свои мелкие проекты, только не забывай про Readme. Потом будешь рад, когда будешь искать проект в котором ты делал работающие аспекты или работу с авторизацией;
- **Ведение заметок.** Очень важный пункт. Тебе предстоит поглощать огромное количество информации, большую часть которой ты забудешь уже на следующей день. Чтобы помочь своему мозгу начни вести заметки по методике ZettelKasten, например в notion.io https://habr.com/ru/post/509756/
...
Java Core это фундамент нашего знания и понимания языка. Это знание и понимание возможностей языка которые у нас есть «из коробки». Так как это фундамент, подойти к его формированию стоит очень ответственно, чтобы не страдать позже. Не стоит пытаться сэкономить время, хитрить или пропускать «неинтересные» куски. Поверь, уверенная база тебе воздастся в будущем, ибо на собеседованиях гоняют в основном по Core.
